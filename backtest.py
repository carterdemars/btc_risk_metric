...
def dynamic_dca_strategy(balance, score, price, start_date):

    buy_and_hold_start = balance/price.loc[price.index == pd.to_datetime(start_date)]['close']
    buy_and_hold = price['close'].apply(lambda x: x*buy_and_hold_start.values[0])
    ...
    # return [total_value.tail(1).values[0], buy_and_hold.tail(1).values[0]]
    ...
    ax1.plot(price.index, bitcoin_owned, color='orange', label="Number of Bitcoin Owned")
    ax1.legend()
    # ax2.set_yscale('log')
    ax2.set_ylim(-1000, 1.2*max(trade_value_over_time.max(), balance_over_time.max()))
    ax2.set_title('Liquid Value')
    ax2.plot(price.index, trade_value_over_time, color='purple', label="Value of BTC Owned in USD")
    ax2.plot(price.index, balance_over_time, color='yellow', label="Cash Balance in USD")
    ax2.legend()
    # ax3.set_yscale('log')
    ax3.set_title('Total Value')

    ax3.set_ylim(0.75* total_value.min(), 1.5*total_value.max())
    ax3.plot(price.index, total_value, color='green', label="Total Value (Strategy)")
    ax3.plot(price.index, buy_and_hold, color='red', label="Total Value (Buy and Hold)")
    ax3.legend()
...
# for date in pd.date_range(date(2016,1,2), date(2022,1,1), freq='d'):
#     score = score.loc[score.index >= pd.to_datetime(date)]
#     price = price.loc[price.index >= pd.to_datetime(date)]
#     strategy_value, buy_and_hold_value = dynamic_dca_strategy(balance, score, price, date)
#
#     if buy_and_hold_value > strategy_value:
#         print(date, strategy_value, buy_and_hold_value)
#     else:
#         print(date, True)
...
dynamic_dca_strategy(balance, score, price, start_date)
